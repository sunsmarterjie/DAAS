...
class Architect(object):
    ...
    def __init__(self, model, args):
        ...
        self.model = model
        self.lr_alpha = args.arch_lr_alpha
        # self.optimizer = torch.optim.Adam(self.model.arch_parameters(),
        #                                   lr=args.arch_learning_rate, betas=(0.5, 0.999),
        #                                   weight_decay=args.arch_weight_decay)
        self.optimizer = torch.optim.Adam([{'params': self.model.arch_parameters(), 'lr': 0},
                                           {'params': self.model.beta_parameters(), 'lr': args.arch_lr_beta}],
                                          betas=(0.5, 0.999),
                                          weight_decay=args.arch_weight_decay)
    ...
    def _backward_step(self, input_valid, target_valid, epoch):
        loss, _, _ = self.model._loss(input_valid, target_valid, epoch)
        loss.backward()
